...
public class ContentResolverHook extends ApiMonitorHook {
	...
	private boolean isSensitiveUri(Uri uri) {
		...
		Logger.log_behavior(ContentResolverHook.class.getName(), url);
	...
	public void startHook() {
		...
		hookhelper.hookMethod(querymethod, new AbstractBehaviorHookCallBack() {
			...
			public void descParam(HookParam param) {
				...
				if (isSensitiveUri(uri)) {
					Logger.log_behavior(ContentResolverHook.class.getName(), "Read ContentProvider -> Uri = " + uri.toString());
					String queryStr = concatenateQuery(uri, (String[]) param.args[1], (String) param.args[2], (String[]) param.args[3],
							(String) param.args[4]);
					Logger.log_behavior(ContentResolverHook.class.getName(), "Query SQL = " + queryStr);
				}
			}
		});
		...
		hookhelper.hookMethod(registerContentObservermethod, new AbstractBehaviorHookCallBack() {
			...
			public void descParam(HookParam param) {
				...
				Uri uri = (Uri) param.args[0];
				if (isSensitiveUri(uri)) {
					Logger.log_behavior(ContentResolverHook.class.getName(), "Register ContentProvider Change -> Uri = " + uri.toString());
					Logger.log_behavior(ContentResolverHook.class.getName(), "ContentObserver ClassName =" + param.args[1].getClass().toString());
				}
			}
		});
		...
		hookhelper.hookMethod(insertmethod, new AbstractBehaviorHookCallBack() {
			...
			public void descParam(HookParam param) {
				...
				Uri uri = (Uri) param.args[0];
				if (isSensitiveUri(uri)) {
					Logger.log_behavior(ContentResolverHook.class.getName(), "Insert ContentProvider -> Uri = " + uri.toString());
					ContentValues cv = (ContentValues) param.args[1];
					String insertStr = concatenateInsert(uri, cv);
					Logger.log_behavior(ContentResolverHook.class.getName(), "Insert SQL = " + insertStr);
				}
			}
		});
		...
		hookhelper.hookMethod(bulkInsertmethod, new AbstractBehaviorHookCallBack() {
			...
			public void descParam(HookParam param) {
				...
				if (isSensitiveUri(uri)) {
					ContentValues[] cv = (ContentValues[]) param.args[1];
					Logger.log_behavior(ContentResolverHook.class.getName(), "Bulk Insert ContentProvider -> Uri = " + uri.toString());
					String insertStr = null;
					for (int i = 0; i < cv.length; i++) {
						insertStr = concatenateInsert(uri, cv[i]);
						Logger.log_behavior(ContentResolverHook.class.getName(), "Insert " + i + " SQL = " + insertStr);
					}
				}
			}
		});
		...
		hookhelper.hookMethod(deletemethod, new AbstractBehaviorHookCallBack() {
			...
			public void descParam(HookParam param) {
				...
				Uri uri = (Uri) param.args[0];
				if (isSensitiveUri(uri)) {
					Logger.log_behavior(ContentResolverHook.class.getName(), "Delete ContentProvider -> uri= " + uri.toString());
					String deleteStr = concatenateDelete(uri, (String) param.args[1], (String[]) param.args[2]);
					Logger.log_behavior(ContentResolverHook.class.getName(), "Delete SQL = " + deleteStr);
				}
		...
		hookhelper.hookMethod(updatemethod, new AbstractBehaviorHookCallBack() {
			...
			public void descParam(HookParam param) {
				...
				Uri uri = (Uri) param.args[0];
				if (isSensitiveUri(uri)) {
					Logger.log_behavior(ContentResolverHook.class.getName(), "Update ContentProvider -> uri=" + uri.toString());
					String updateStr = concatenateUpdate(uri, (ContentValues) param.args[1], (String) param.args[2], (String[]) param.args[3]);
					Logger.log_behavior(ContentResolverHook.class.getName(), "Update SQL = " + updateStr);
				}
			}
		});
		...
		hookhelper.hookMethod(applyBatchMethod, new AbstractBehaviorHookCallBack() {
			...
			public void descParam(HookParam param) {
				...
				for(int i=0; i< opts.size(); i++){
					Logger.log_behavior(ContentResolverHook.class.getName(), "Batch SQL = " + descContentProviderOperation(opts.get(i)));
				}
